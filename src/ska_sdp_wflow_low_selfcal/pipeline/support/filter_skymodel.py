...
from ska_sdp_wflow_low_selfcal.pipeline.support.observation import Observation
...
def calc_theoretical_noise(mslist, w_factor=1.5):
    ...
    # average unflagged fraction, and average number of core and remote
    # stations (for the averages, assume each observation has equal weight)
    total_time = 0
    total_bandwidth = 0
    ncore = 0
    nremote = 0
    ...
    for ms in mslist:
        ...
        nremote += len(
            [stat for stat in obs.stations if stat.startswith("RS")]
        )
    ...
    noise = w_factor / np.sqrt(
        2
        * (2 * total_time * total_bandwidth)
        * (core_term + mixed_term + remote_term)
    )  # Jy
...
def find_unflagged_fraction(ms_file):
    ...
    if (sys.version_info.major, sys.version_info.minor) >= (3, 7):
        ...
        result = subprocess.run(
            "taql 'CALC sum([select nfalse(FLAG) from {0}]) / "
...
def filter_skymodel(
    ...
    theoretical_rms, unflagged_fraction = calc_theoretical_noise(
        beamMS
    )  # Jy/beam
        ...
        if input_bright_skymodel_pb is not None:
            try:
                ...
                s_bright = lsmtool.load(
                    input_bright_skymodel_pb, beamMS=beamMS[beam_ind]
                )
    ...
    if not emptysky:
        if comparison_skymodel is None:
            ...
            try:
                s_comp = lsmtool.load(
                    "tgss", VOPosition=[midRA, midDec], VORadius=5.0
                )
            except OSError:
        ...
        if s_comp is not None:
            ...
            # Check if there is a sufficient number of sources to do the
            # comparison with. If there is, do it and append the resulting
            # diagnostics dict to the existing one
            #
            # Note: the various ratios are all calculated as (s_in / s_comp)
            # and the differences as (s_in - s_comp). If there are no
            # successful matches, the compare() method returns None
            if (
                s_comp
                and len(s_in.getPatchNames()) >= 10
                and len(s_comp.getPatchNames()) >= 10
            ):
                flux_astrometry_diagnostics = s_in.compare(
                    s_comp,
                    radius="5 arcsec",
    ...
    if emptysky:
        ...
        sdec = (
            ...
            + str(dec[1]).zfill(2)
            + "."
            + str("%.6f" % (dec[2])).zfill(6)
        )
        dummylines.append(",,p1,{0},{1}\n".format(sra, sdec))
        dummylines.append(
            "s0c0,POINT,p1,{0},{1},0.00000001,"
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "input_skymodel_pb", help="Filename of input sky model"
    )
    ...
    parser.add_argument(
        "--input_bright_skymodel_pb",
        help="Filename of input bright-source sky model",
        type=str,
        default=None,
    )
    parser.add_argument(
        "--threshisl", help="Island threshold", type=float, default=3.0
    )
    parser.add_argument(
        "--threshpix", help="Peak pixel threshold", type=float, default=5.0
    )
    parser.add_argument(
        "--rmsbox",
    ...
    parser.add_argument(
        "--adaptive_rmsbox",
        help="Use an adaptive rms box",
        type=str,
        default="True",
    )
    parser.add_argument(
        "--beamMS",
        help="MS filename to use for beam attenuation",
        type=str,
        default=None,
    )
    ...
    filter_skymodel(
        args.input_image,
        args.input_skymodel_pb,
        args.output_skymodel,
        args.vertices_file,
        input_bright_skymodel_pb=args.input_bright_skymodel_pb,
        threshisl=args.threshisl,
        threshpix=args.threshpix,
        rmsbox=args.rmsbox,
        rmsbox_bright=args.rmsbox_bright,
        adaptive_rmsbox=args.adaptive_rmsbox,
        beamMS=args.beamMS,
    )
